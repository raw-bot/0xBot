...
class RiskManagerService:
    ...
    def validate_entry(
        ...
        try:
            ...
            size_pct = Decimal(str(decision.get("size_pct", config.DEFAULT_POSITION_SIZE_PCT)))
            ...
            existing_position = any(
                pos.symbol == symbol and pos.status == PositionStatus.OPEN
            ...
            entry_price = decision.get("entry_price") or decision.get(
                "price"
            )  # LLM might provide entry_price or price
            ...
            if side == "long":
                ...
                stop_loss_pct = (
                    (stop_loss_price - entry_price) / entry_price
                    if entry_price > 0
                    else Decimal("0")
                )
                take_profit_pct = (
                    (entry_price - take_profit_price) / entry_price
                    if entry_price > 0
                    else Decimal("0")
                )
            ...
            if side == "long":
                if not (stop_loss_price < entry_price < take_profit_price):
                    return (
                        False,
                        f"Invalid price relationships for LONG: SL({stop_loss_price:,.2f}) < ENTRY({entry_price:,.2f}) < TP({take_profit_price:,.2f})",
                    )
            else:  # short
                if not (take_profit_price < entry_price < stop_loss_price):
                    return (
                        False,
                        f"Invalid price relationships for SHORT: TP({take_profit_price:,.2f}) < ENTRY({entry_price:,.2f}) < SL({stop_loss_price:,.2f})",
                    )
            ...
            if stop_loss_pct <= 0:
                return (
                    False,
                    f"Stop loss percentage must be positive (calculated: {stop_loss_pct:.4f})",
                )

            if take_profit_pct <= 0:
                return (
                    False,
                    f"Take profit percentage must be positive (calculated: {take_profit_pct:.4f})",
                )
    ...
    def check_drawdown(bot: Bot, current_portfolio_value: Decimal) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if drawdown >= warning_threshold:
                return (
                    True,
                    f"Approaching max drawdown: {drawdown:.1%} (max: {max_drawdown_pct:.1%})",
                )
    ...
    def calculate_position_size(
        ...
        """
        Calculate position size in base currency with confidence-based optimization.

        Args:
            capital: Available capital
            size_pct: Base position size as percentage (0.10 = 10%)
            current_price: Current market price
            confidence: LLM confidence level (0.0 to 1.0) - if provided, adjusts position size
        ...
        # Check for positive capital
        if capital <= 0:
            logger.warning(
                f"⚠️  RiskManager | Negative or zero capital: ${capital:,.2f}. Cannot open position."
            )
            return Decimal("0")
        ...
        # Adjust position size based on confidence if provided
        if confidence is not None:
            confidence = Decimal(str(confidence))

            # Confidence-based position sizing (Kelly-like approach)
            # Scale position from 50% to 120% of base size based on confidence
            min_confidence_adj = Decimal("0.5")  # 50% of base for very low confidence
            max_confidence_adj = Decimal("1.2")  # 120% of base for very high confidence

            # Map confidence 0.3-0.9 to adjustment 0.5-1.2
            if confidence < Decimal("0.3"):
                adjustment = min_confidence_adj
            elif confidence > Decimal("0.9"):
                adjustment = max_confidence_adj
            else:
                # Linear interpolation: (confidence - 0.3) / (0.9 - 0.3) = (confidence - 0.3) / 0.6
                normalized_confidence = (confidence - Decimal("0.3")) / Decimal("0.6")
                adjustment = (
                    min_confidence_adj
                    + (max_confidence_adj - min_confidence_adj) * normalized_confidence
                )

            position_value *= adjustment
            logger.info(
                f"📊 Position size adjusted by confidence {confidence:.0%}: {adjustment:.1%} of base"
            )
    ...
    def calculate_stop_loss_price(
        entry_price: Decimal, stop_loss_pct: Decimal, side: str
    ) -> Decimal:
        """
    ...
    def calculate_take_profit_price(
        entry_price: Decimal, take_profit_pct: Decimal, side: str
    ) -> Decimal:
        """
