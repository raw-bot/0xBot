...
class OHLCV:
    ...
    def __repr__(self) -> str:
        return (
            f"<OHLCV({self.datetime}, O:{self.open}, H:{self.high}, L:{self.low}, C:{self.close})>"
        )
...
class MarketDataService:
    ...
    async def fetch_ticker(self, symbol: str) -> Ticker:
        ...
        try:
            ...
            if not ticker_data.get("last"):
                logger.warning(f"⚠️ RAW TICKER MISSING LAST PRICE for {symbol}: {ticker_data}")
            else:
                logger.info(f"✅ Ticker {symbol}: {ticker_data.get('last')}")
    ...
    async def get_open_interest(self, symbol: str) -> dict:
        ...
        try:
            ...
            return {"latest": latest, "average": average}
            ...
            snapshot = {
                ...
                "technical_indicators": {
                    ...
                    "1h": {  # For longer-term context
                        ...
                        "rsi14": 50,
                        "atr3": (
                            atr3_series[-1] if atr3_series and atr3_series[-1] is not None else 0
                        ),
                        "atr14": (
                            atr14_series[-1] if atr14_series and atr14_series[-1] is not None else 0
                        ),
    ...
    async def get_market_data_multi_timeframe(
        ...
        """
        logger.debug(
            f"Fetching multi-timeframe data for {symbol}: {timeframe_short} + {timeframe_long}"
        )
